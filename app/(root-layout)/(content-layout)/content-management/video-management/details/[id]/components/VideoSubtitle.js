...
import {subTitleDelete} from "@/utils/api/videoContentRequest";
const VideoSubtitle = ({video}) => {
    ...
    const deleteSubTitle = async (id) => {
        const response = await subTitleDelete(id, video.id)
        if (response.status === "success") {
            setSubTitles((old) => (old.filter((subtitle) => subtitle.id !== id)))
        }
    }
    ...
    return (
        <div>
            <div className="inner">
                ...
                <div className="max-h-[341px] overflow-hidden bg-secondaryBgColor pr-1">
                    <div className="flex flex-col custom-subtitle-scrollbar pr-1 relative max-h-[341px]">
                        <div className=" inset-0">
                            {subTitles.length ? subTitles.map((subtitle, index) => (
                                    <FieldWrapper label={subtitle.language} className={`relative ${index === subTitles.length - 1
                                        ?"border-b border-commonBorderColor" : ""}`} singleElement={true} key={`key-${index}`}>
                                        <div className="flex items-center justify-between py-2 pr-[52px]">
                                            ...
                                            <div onClick={()=>deleteSubTitle(subtitle.id)} className="cursor-pointer absolute w-[52px] flex items-center justify-center right-0 bg-secondaryBgColor h-full border-l border-commonBorderColor">
                                                <Trash2 size={20} />
                                            </div>
                                        </div>
                                    </FieldWrapper>
                                ...
                                :
                                <div className={"relative justify-center flex items-center max-h-[300px]"}>
