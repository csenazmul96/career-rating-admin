...
const CurriculumCategoryPicker = ({isOpen,
                                      setIsOpen,
                                      selectedGroup = null,
                                      categories,
                                      callBackFunction = () => {},
    ...
    const hasThirdLevel = (group) => group.subCategories.some(
        subGroup => Array.isArray(subGroup.subCategories) && subGroup.subCategories.length > 0
    );
                    ...
                    <div className="border border-borderColor">
                        <ul className={`max-h-[240px] custom-siderbar-scrollbar flex flex-col`}>
                            {categories.map((item, index) => (
                                <li key={index} className={`flex flex-col ${!hasThirdLevel(item) && item.subCategories.length ? 'to-2nd-level' : ''}`}>
                                    <div
                                        onClick={() => {
                                    ...
                                    {item.subCategories?.length ? (
                                        <ul className={`ml-[48px] ${!hasThirdLevel(item) && item.subCategories.length ? 'to-2nd-level ul !ml-[24px]' : ''} ${hasThirdLevel(item) && item.subCategories.length ? 'ml-[44px]' : ''}`}>
                                            {item.subCategories.map((subItem) => (
                                                <li key={subItem.id}
                                                    className={` ${!subItem.subCategories.length ? "h-[36px] flex items-center py-[8px]" : "hello not"} hover:border-themeColor hover:text-themeColor  text-textSubColor font-normal text-[13px] ${!hasThirdLevel(item) && item.subCategories.length ? ' py-[8px] px-[20px] border-l border-borderColor hover:border-themeColor hover:text-themeColor hover:bg-[#F4F9FF]   text-textSubColor font-normal text-[13px]' : ''} ${hasThirdLevel(item) && item.subCategories.length ? 'to-3nd-level li' : ''}` }>
                                                    {subItem.subCategories.length ? (
                                                            <>
                                                                <div
                                                                    ...
                                                                    className={`${getActiveStatus(2) === subItem.id ? '  text-themeColor border-themeColor' : ''} ${hasThirdLevel(item) && item.subCategories.length ? 'to-3nd-level pl-0' : ''}  cursor-pointer flex items-start justify-between gap-3 text-textSubColor  text-[13px]  hover:text-themeColor h-[36px]  py-[8px] px-4`}>
                                                                ...
                                                                {currentSubGroup === subItem.id &&
                                                                <ul className="">
                                                                    {subItem.subCategories.map((third, i) => (
                                                                        <li key={`third ${third.id}`}
                                                                            className={`${getActiveStatus(3) === third.id ? 'bg-[#F4F9FF] text-themeColor' : ''} cursor-pointer py-[5px] px-[21px] border-l border-borderColor hover:border-themeColor hover:text-themeColor hover:bg-[#F4F9FF]  text-textSubColor font-normal text-[13px]`}>
                                                                        <span
                                                                            className={`border-themeColor`}
                                                                            onClick={() => {
                                                                                callBackFunction({
                                                                                    ...third,
                                                                                    parentId: item.id,
...
export default CurriculumCategoryPicker