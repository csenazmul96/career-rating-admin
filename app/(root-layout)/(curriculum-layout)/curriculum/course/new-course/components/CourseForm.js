...
export default function CourseForm({ oldData = null }) {
  ...
  const submitHandler = async () => {
    ...
    if (response.status === 500) {
      CommonToastMessage("오류.", response.error, "error");
      setLoading(false);
      return false;
    }
    ...
    if (response.status === "error") {
      ...
      if (errors?.courseId) {
        CommonToastMessage("오류.", errors.courseId, "warning");
      }
      if (errors?.id) {
        CommonToastMessage("오류.", errors.id, "warning");
      }
    } else {
      if (oldData) {
        CommonToastMessage(
        ...
        CommonToastMessage("등록 완료", "강좌가 등록되었습니다.", "success");
