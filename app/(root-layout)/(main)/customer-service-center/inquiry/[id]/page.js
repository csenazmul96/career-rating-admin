import InquiryReplyDeleteButton from "@/app/(root-layout)/(curriculum-layout)/curriculum/course/details/[id]/lecture-inquiry/[slug]/components/InquiryReplyDeleteButton";
import InqueryReplyForm from "@/app/(root-layout)/(main)/customer-service-center/inquiry/[id]/components/InqueryReplyForm";
      ...
      {query && (
        <>
          <div className="border-t border-commonBorderColor">
            <div className="group px-5 py-4 flex w-full items-center justify-between bg-secondaryBgColor">
              <div className="inner">
                <span className="text-medium font-bold">
                  <span className={`text-themeColor font-bold`}>Q.</span>{" "}
                  {query.title}{" "}
                </span>
              </div>
              <div className="inner">
                <ul className="flex gap-4 text-textSubColor">
                  <li className={`text-base flex items-center gap-1`}>
                    <span>
                      <img
                        src="/images/curriculum-management/person.png"
                        alt=""
                      />
                    </span>
                  ...
                  <li className={`text-base flex items-center gap-1`}>
                    <span>
                      <Clock className={`text-[#8E8E8E] size-[15px]`} />
                    </span>
                    <span>{query.lastResponseDate}</span>
                  </li>
                </ul>
              </div>
            </div>
          ...
          {query.inquiryReplyList.map((reply) => (
            <div
              className="flex flex-col p-8 gap-4 border-b border-commonBorderColor"
              key={reply.id}
            >
              <div className="avatar-info flex items-start justify-between gap-2">
                <div className="text">
                  <p className={`text-base font-bold`}>{reply.createdBy}</p>
                  <p className={`text-base text-textSubColor`}>
                    {reply.createdAt}
                  </p>
                  <p className={`pt-3`}>{reply.details}</p>
                </div>
                <InquiryReplyDeleteButton
                  url={`/student-inquiry/reply/${reply.id}`}
                />
              </div>
                    ...
                    {reply.attachmentFiles.map((file, i) => (
                      ...
                      >
                        <div className="left flex items-center gap-3">
                          <span>
                            <img
                              src={`/images/content-management/${getFileExtension(
                                file.fileType
                              )}`}
                              alt=""
                            />
                        ...
                        <div className="right flex gap-3 items-center">
                          ...
                          <Link href={file.downloadLink} download={true}>
                            <span className="text-textSubColor cursor-pointer">
                              <MdOutlineDownload />
                            </span>
                          </Link>
          ...
          <div className="flex items-end justify-end pt-10">
            <Link
              href={"/customer-service-center/inquiry"}
              className={"flex"}
              color="transparent"
            >
              <span>
