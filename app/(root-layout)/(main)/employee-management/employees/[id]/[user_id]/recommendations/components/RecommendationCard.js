...
import {deleteRecommendations} from "@/utils/api/career/recommendationsAPI";


function RecommendationCard({recommendation, userId, id, hangleCreateNewButtonClick}) {
    const deleteRecommendation = async () => {
        confirmAlert({
            title: "Delete",
            message:
                "Do you really want to delete this Recommendation?",
            buttons: [
                {
                    label: "Cancel",
                    buttonLabel: "Cancel",
                ...
                },
                {
                    ...
                    onClick: async () => {
                        try {
                            const request = await deleteRecommendations(recommendation.id);
                            if (request && request.status === 200) {
                                LmsToastMessage('Delete', 'Recommendation has been deleted successfully.', 'success')
                            }
                        } catch (error) {
        ...
        <div className={`shadow-dashboardShadow relative flex-1 min-w-0 bg-white py-[1.625rem] px-8 group`}>
            <div className="flex gap-x-4   ">
                ...
                <div className="text flex flex-col gap-1 w-full">
                    <p className={`text-baseNormal text-textSubColor`}>{recommendation.name}</p>
                    <span>{recommendation.email}</span>
                    <span className={`text-13 text-textColor`}>
            ...
            <div className="opacity-0 absolute right-2 top-2 invisible transition-opacity duration-200 group-hover:opacity-100 group-hover:visible">
                <Button onClick={()=>hangleCreateNewButtonClick(recommendation)} color={"primaryLightRoundedSmall"} className={'!h-10  rounded-full'}><Pencil size={16} /> </Button>
            </div>
            <span className="opacity-0 absolute right-2 bottom-2 invisible transition-opacity duration-200 group-hover:opacity-100 group-hover:visible">
                <Button onClick={deleteRecommendation} color={"primaryLightRoundedSmall"} className={'!h-10  rounded-full !text-dangerDeppColor'}><Trash2 size={16} /> </Button>
            </span>
        </div>
    );
}
